...
describe("obfuscateData", () => {
  ...
  it("shoud obfuscate as we are passing only string", () => {
    const data = { person: { email: "example@gmail.com" } };
    const newDate = mask(data, ["person.email"]);
    expect(newDate).toEqual({ person: { email: "*****************" } });
  });

  it("shoud obfuscate as we are passing number or boolean", () => {
    const data = { person: { age: 25, isBrazilian: true } };
    const newDate = mask(data, ["person.age", "person.isBrazilian"]);
    expect(newDate).toEqual({ person: { age: "**", isBrazilian: "*" } });
  });
});
